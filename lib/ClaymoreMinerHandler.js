...
class ClaymoreMinerHandler {
  ...
  constructor(minerToSpawn, minerArgs, currentCoin, minerHost = "localhost", minerPort = 3333) {
    ...
    // wait for first ping when spawning miner ping...wait for miner for a minute
  ...
  ping() {
    ...
    return Promise.all([this.identifyMiner(), this.parseMinerStats()])
      .then((minerIdentity) => {
        ...
        this.pingRetries = minerIdentity[0] && minerIdentity[1] ? 0 : this.pingRetries;
        return minerIdentity;
      })
      .catch((err) => {
        console.log(`Could not get ping...${this.pingRetries} retries attempted`);
        this.pingRetries += 1;
        if(err.port === this.minerPort && this.pingRetries === 5) {
  ...
  swapMiner(minerToSpawn, minerArgs, maxProfitTag = "") {
    ...
    if (this.chosenMiner === minerToSpawn && this.currentCoin === maxProfitTag) {
      console.log("Already mining this coin");
      return null;
    }
    ...
    if (os.platform() === "win32") {
      ...
      if (!this.minerProcess) {
        this.findWindowsMinerProcesses()
          ...
          .catch((err) => {
            console.log("no win proc to kill");
          });
      } else {
