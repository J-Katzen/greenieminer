        ...
        _.forEach(minerStats.result, (stat, idx) => {
          switch (idx) {
            case 0:
              ...
              break;
            case 1:
              parsedStats.uptime = stat;
              break;
            case 2:
              let splitHashShares0 = stat.split(';');
              parsedStats.totalHash_0 = stat[0];
              parsedStats.totalShares_0 = stat[1];
              parsedStats.rejectedShares_0 = stat[2];
              break;
            case 3:
              let splitGPUHashes0 = stat.split(';');
              _.forEach(splitGPUHashes0, (hash, gpuIdx) => parsedStats[`GPU${gpuIdx}_0`] = hash);
              break;
            case 4:
              let splitHashShares1 = stat.split(';');
              parsedStats.totalHash_1 = splitHashShares1[0];
              parsedStats.totalShares_1 = splitHashShares1[1];
              parsedStats.rejectedShares_1 = splitHashShares1[2];
              break;
            case 5:
              let splitGPUHashes1 = stat.split(';');
              _.forEach(splitGPUHashes1, (hash, gpuIdx) => parsedStats[`GPU${gpuIdx}_1`] = hash);
              break;
            case 6:
              let splitTempFans = stat.split(';');
              _.forEach(splitTempFans, (tempFan, idx) => {
                if (idx % 2 === 0) {
                  parsedStats[`GPU${idx}_temp`] = tempFan;
                } else {
                  parsedStats[`GPU${idx-1}_fan`] = tempFan;
                }
              });
              break;
            case 7:
