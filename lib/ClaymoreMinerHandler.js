...
class ClaymoreMinerHandler {
  ...
  ping() {
    ...
    return Promise.all([this.identifyMiner(), this.parseMinerStats()])
      ...
      .catch((err) => {
        ...
        this.pingRetries += 1;
        if(err.port === this.minerPort && this.pingRetries === 3) {
  ...
  swapMiner(minerToSpawn, minerArgs, maxProfitTag = "") {
    ...
    const currentSortedArgs = this.minerArgs.concat().sort();
    const incomingSortedArgs =  minerArgs.concat().sort();
    const minerArgsSame = _.isEqual(currentSortedArgs, incomingSortedArgs);
    if (this.chosenMiner === minerToSpawn && minerArgsSame  && this.initialized) {
