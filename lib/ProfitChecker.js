...
class ProfitChecker {
  ...
  createWTMRequset(fullUrl = false) {
    let profitUrl = '';
    profitUrl = fullUrl ? this.buildFullUrl() : this.buildProfitUrl();
  ...
  getCoinProfitability() {
    return this.createWTMRequset().then(this.determineMostProfit.bind(this))
      .catch(function(err) {
        console.log(err);
  ...
  getCoinsByAlgo(algo) {
    return this.createWTMRequset(true).then(function(coinResponse) {
      return _.filter(coinResponse.coins, function(coin) {
        return wtmQueryParamMap[algo].name === coin.algorithm;
      });
    });
  }
  ...
  buildFullUrl() {
    let queryMap = {};
    _.forEach(wtmQueryParamMap, function(algo) {
      queryMap[algo.qp] = true;
    });
    const queryParamString = encodeQueryData(queryMap);
