...
class ProfitabilityChecker {
  ...
  determineMostProfit(coinResponse) {
    ...
    for (coinName in coins) {
      const isEnabledCoin = enabledCoinTags.indexOf(coins[coinName].tag) > -1;
  ...
  buildProfitUrl() {
    ...
    for (key in this.algoHashes) {
      ...
      if (this.algoHashes.hasOwnProperty(key)) {
        ...
        queryMap[`factor[${factoryKey}_hr`] = this.algoHashes.hasOwnProperty(key);
      }
    }
