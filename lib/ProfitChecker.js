...
class ProfitChecker {
  ...
  constructor(algoHashes = {}, enabledCoinTags = [], cost = 0.1) {
  ...
  buildProfitUrl() {
    ...
    for (let key in this.algoHashes) {
      ...
      queryMap["factor[cost]"] = this.cost;
      if (this.algoHashes.hasOwnProperty(key) && this.algoHashes[key] !== 0) {
        const factorKey = wtmQueryParamMap[key].factorPrefix || wtmQueryParamMap[key].qp;

        queryMap[`factor[${factorKey}_hr]`] = this.algoHashes[key];
      }
    }
    const queryParamString = querystring.stringify(queryMap);
  ...
  buildFullUrl() {
    ...
    const queryParamString = queryMap.stringify(queryMap);
